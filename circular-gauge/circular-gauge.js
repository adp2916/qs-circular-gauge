define(["qlik", "./circular-gauge-d3-0.1", "./gauge-props","./fs-icon-array","./color-picker"],
	...
	if(!$("link[id='FA']").length > 0){
		$( '<link id="FA" rel="stylesheet" type="text/css" href="https://use.fontawesome.com/releases/v5.13.0/css/all.css">' ).appendTo( 'head' ); // Font Awesome CDN		
	};
		return {
			initialProperties: {
				qHyperCubeDef: {
			...
			paint: function ($element, layout) {
				...
				let kpi1 = props.kpi1;
				let kpi2 = {  kpiLabel : props.kpi2Label,
							  kpiLabelIcon : '',
							  kpi      :props.kpi2,
				...
				let config = {
					...
					dialDesign: {
						...
						dialRingFillColor : (props.dialRingFillColor?props.dialRingFillColor:0),
						dialRingStrokeColor : (props.dialRingStrokeColor?props.dialRingStrokeColor:0),
				...
				let currentSize = $("[tid=" + id + "]").height() + "-" + $("[tid=" + id + "]").width();
				//console.log(id, currentSize, prevSize)
				// 
				if (!vSvg.length || editMode.length || currentSize != prevSize["q" + id]) {
					...
					$element.append('<div id="fa-'+id+'-container" style="display:none;justify-content:center;align-items:center;overflow:scroll;flex-flow:row;position:fixed;top:0px;left:0px;height:100vh;width:100%;background:rgba(256,256,256,0);z-index:126"></div>')
					$('#fa-'+id+'-container').append('<div id="fa-'+id+'" style="overflow:scroll;flex-flow:row;height:500px;width:600px;padding:20px;background:rgba(256,256,256);z-index:126"></div>')
					
					for (let key in fa.fontAwesome) {
						
						// check if the property/key is defined in the object itself, not in parent
						$('#fa-'+id).append('<i title="Click me to copy to clipboard!" data-key = "'+fa.fontAwesome[key][0]+'" class="fa '+key+'" style="cursor:pointer;padding:5px;margin:5px;border:solid 1px gray"></i>')
					};
					$('#fa-'+id).append('<input type="text" value="Icons" class="myInput" >');
					$('#fa-'+id+'-container').on('click','i',getICPicker)

					$element.append('<div id="color-'+id+'-container" style="display:none;justify-content:center;align-items:center;overflow:scroll;flex-flow:row;position:fixed;top:0px;left:0px;height:100vh;width:100%;background:rgba(256,256,256,0);z-index:126"></div>')
					$('#color-'+id+'-container').append('<div id="color-'+id+'" style="display:flex;flex-wrap:wrap;overflow:scroll;height:500px;width:600px;padding:20px;background:rgba(256,256,256);z-index:126"></div>')
					for (let j=0; j<c.colors.length; j++) {
						
						// check if the property/key is defined in the object itself, not in parent
						$('#color-'+id).append('<div class="colorPicker" title="Click me to copy to clipboard!" data-key = "'+c.colors[j]+'"  style="display:flex;background:'+c.colors[j]+';cursor:pointer;padding:5px;margin:5px;border:solid 1px gray;width:90px;height:30px"></div>')
					};
					$('#color-'+id).append('<input type="text" value="color" class="myInput" style="width:550px;height:20px" >');

					$('#color-'+id+'-container').on('click','div.colorPicker',getColorPicker);
				...
				$("[tid=" + id + "] header>h1").css('justify-content',alignment[props.headerFooterAlignment]);
				...
				$("[tid=" + id + "] footer").css('justify-content',alignment[props.headerFooterAlignment]);
				$("[tid=" + id + "] footer").css('color',props.footerColor);
				$("[tid=" + id + "] .qv-footer-wrapper").css('background',props.footerBgColor);	
