    ...
    let gauge = function (element, config) {
       ...
       // console.log("endAngle", endAngle)
        ...
        let yPosStartAngle = function () {
            if (endAngle < 90) {
                return h - ((h / 2) + (r + gWidth) * Math.sin(centerAdjustMent * dTor)) - 10;
            }
            else if ((endAngle < 105)) {
                return h - ((h / 2) + (r + gWidth) * Math.sin(startTextAngle * dTor)) - 10;
            }
        ...
        let tickData = d3.range(majorTicks).map(function () { return maxVal / majorTicks; });
        ...
        let mainArc = gViz.append('g').selectAll('path')
        ...
        let ticksText = gViz.append('g').selectAll('text')
            .data(tickData)
            .enter()
            .append('text')
            .text(function (d, i) {
            ...
            .attr('text-anchor', function (d, i) {
                ...
                if (textAngle > (midPoint - 15) && textAngle < (midPoint + 15)) {
                    ...
                    return (textAngle <= (startTextAngle + (tickSpan * (majorTicks / 2)))) ? 'end' : 'start';
                }
        ...
        let centerKPI = gViz.append('g')
                        .attr('id','d-center-kpi')
        let kpiText = centerKPI.append('text')
        
        let kpiLabel = centerKPI.append('text')
      
        
        let kpiIcon = centerKPI.append('text')
        .attr("class", "fa");
        ...
        this.setCercularKpi = function (kpi,dKpi,config) {
            ...
            let _yTransform = (yTransform>h/2) ? yTransform-((yTransform-h/2)/1.5) : yTransform;
            let angle;
            ...
            if(needleDesign.showNeedle==true){
                ...
                kpiArcPath.transition()
                .duration(750)
                .attrTween("d", arcTween(angle*dTor));
                if(_config.centerKpi.show==true){
                    let icon = String.fromCharCode(parseInt(_config.centerKpi.labelIcon,16));
                    let xAdjustmentForIcon = (_config.centerKpi.labelIcon.length>1  && _dKpi.kpiLabel.length>1? kpiTextheight/1.5: 0);
                    let iconAnchor = (_config.centerKpi.labelIcon.length>1 && _dKpi.kpiLabel>1? 'right': 'middle');
                    let labelAnchor = (_config.centerKpi.labelIcon.length>1 && _dKpi.kpiLabel>1? 'left': 'middle');
                   // console.log('_config.centerKpi.labelIcon.length',xAdjustmentForIcon)
                    kpiText.text(_dKpi.kpi)
                    .attr('x',xTransform)
            ...
            if(_config.dialDesign.showDialFullRing===true){
                ...
                gVizRing.transition().duration(500)
